...
#define BRIGHTNESS            255          // built-in with FastLED, range: 0-255 (recall that each pixel uses ~60mA when set to white at full brightness, so full strip power consumption is roughly: 60mA * NUM_LEDs * (BRIGHTNESS / 255)
...
#define   SUPER_CONTROLLER_ID 302673429    // this gives you a special node id that changes the animation.  I'm using it for an art car as a special node in the mesh.  Acknowledging that this being hardcoded is not ideal.
...
uint8_t aloneHue = random(80,191);       // random color set on each reboot, used for the color in the "alone" animation, 223 gives room for a random number 0-32 to be added for confetti effect.
uint8_t animationDelay = 12;  // random animation speed, between (x,y) milliseconds, used to create a unique color/vibration scheme for each individual light when in "alone" mode
...
void setupLEDs() {
  // create an LED object and set brightness
...
void confetti() {
  ...
  leds[pos] += CHSV(aloneHue + random8(64), 200, 255);
}
 
void banana_mode() {
  // slight hue shift, but SO SO YELLOW
